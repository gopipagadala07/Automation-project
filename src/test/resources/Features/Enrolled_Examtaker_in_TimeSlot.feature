Feature: FocalPointk12 Exam_Center Procter Comment, Reset the Examination
  ...
  Scenario Outline: Provide the Procter Comment to a ExamTaker
    ...
    Then User Select the Location <Examinations_details_rownumber>
    When User select the particular Timeslot <Timeslot_rownumber>
    Then User search the particular ExamTaker <Examinations_details_rownumber>
    When Examtaker not added in the Timeslot then add the Examtaker in the Timeslot
    Then User search the added ExamTaker in the TimeSlot <Examinations_details_rownumber>
    ...
    Examples: 
      | rownumber | Examinations_details_rownumber | Timeslot_rownumber |
      |         1 |                              0 |                  0 |
