...
//		dataToInsert.add(new String[]{All_User_CreationPage.ControllerF_Name,String.valueOf(All_User_CreationPage.ControllerL_Name)});
//		
...
//
//	}
	public void InsertmultipledataIntoExcel(String Path, String Sheet, String Value1, String Value2, String Value3,String Value4, String Value5, String Value6) throws Exception
	{
		List<String[]> dataToInsert = new ArrayList<>();
		//dataToInsert.add(new String[]{Schoolname, ClassroomName, SectionName});
		dataToInsert.add(new String[]{Value1,Value2});
		dataToInsert.add(new String[]{Value3,Value4});
		dataToInsert.add(new String[]{Value5,Value6});	
	...
	public void selectFutureRandomTime(WebElement timePickerElement) {
	    ...
	    while (attempt < maxRetries && !success) {
	        try {
	            ...
	            StaticWait(1);
	            Ehour=1+futureHour;
	            int hourDegree = Ehour * 30; // Normalize to 12-hour clock if required
	            String hourDegreeTransform = "rotateZ(-" + hourDegree + "deg)";
	            String hourXpath = "//button[contains(@style,'transform: " + hourDegreeTransform + ";')]";
	            WebElement hourElement = wait.until(ExpectedConditions.elementToBeClickable(By.xpath(hourXpath)));
	            Actions a = new Actions(driver);
